...
- `kitty.conf` sets `shell /usr/bin/fish` explicitly. Without it, kitty launches whatever the user's login shell is (often `/bin/bash` on a fresh install), which silently skips every fish alias/prompt/function in this rice.
